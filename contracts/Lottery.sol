...
contract Lottery is VRFConsumerBase {
    ...
    // All the funds for each lottery game
    mapping (uint => uint) public lotteryBalances;

    // Lottery key for the latest lottery game (made for callback during winner choice)
    ...
    function payLotteryWinner(uint lotteryKey, uint random) private {
        uint winner = random % lotteryGamblers[lotteryKey].length;
        lotteryGamblers[lotteryKey][winner].transfer(
            lotteryBalances[lotteryKey] // all the money in one lottery game
        );
        lotteryHistory[lotteryKey] = lotteryGamblers[lotteryKey][winner];
    }
